...
- When the user asks to check or evaluate UI design, run `openwolf designqc` to capture screenshots, then read them from `.wolf/designqc-captures/`.
- When the user asks to change, pick, or migrate UI framework, read `.wolf/reframe-frameworks.md`, ask decision questions, recommend a framework, then execute with the framework's prompt.
